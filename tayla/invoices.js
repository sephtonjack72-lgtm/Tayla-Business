  ...
  tbody.innerHTML = filtered.map(inv => {
    ...
    return `
      <tr>
        ...
        <td style="color:${status === 'overdue' ? 'var(--danger)' : 'inherit'};font-weight:${status === 'overdue' ? '600' : '400'};">${inv.due_date ? fmtDate(inv.due_date) : '—'}</td>
        ...
        <td>
          <div class="flex-gap">
            ...
            ${['sent','overdue'].includes(status) && contact?.email ? `<button class="btn btn-ghost btn-sm" style="color:var(--accent3);" onclick="sendInvoiceReminder('${inv.id}')" title="Send payment reminder">📧</button>` : ''}
...
function openPrintWindow(inv) {
  ...
  const bankDetails = typeof getBankDetails === 'function' ? getBankDetails() : {};
  ...
  const hasBankDetails = bankDetails.bsb || bankDetails.accountNumber;
  const bankHtml = hasBankDetails ? `
    <div class="notes" style="margin-top:24px;">
      ...
      ${bankDetails.accountName ? `Account Name: ${bankDetails.accountName}<br>` : ''}
      ${bankDetails.bsb ? `BSB: ${bankDetails.bsb}<br>` : ''}
      ${bankDetails.accountNumber ? `Account Number: ${bankDetails.accountNumber}<br>` : ''}
      ${bankDetails.paymentRef ? `<br>${bankDetails.paymentRef}` : `Please use <strong>${inv.number}</strong> as your payment reference.`}
    </div>` : '';

  const logoHtml = profile.logo_base64
    ? `<img src="${profile.logo_base64}" style="max-height:60px;max-width:200px;object-fit:contain;margin-bottom:8px;">`
